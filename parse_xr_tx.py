...
def process_xr_payload():
    ...
    # Apply the exact XR mask [4, 4, 0, 0]
    xor_mask = bytes([4, 4, 0, 0])
    decoded_payload = bytes(b ^ xor_mask[i % 4] for i, b in enumerate(raw_payload))
    
    payload_hex = decoded_payload.hex()
    print(f"[*] XR Mask Applied Successfully")
    print(f"[*] Resulting Hex Length: {len(decoded_payload)} bytes")
    print(f"[*] Resulting Hex: {payload_hex}")

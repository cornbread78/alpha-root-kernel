...
def run_pure_payload():
    ...
    # Pure decode with ZERO structural mutations
    decoded_payload = bytes(b ^ xor_mask[i % 4] for i, b in enumerate(raw_payload))
    pure_hex = decoded_payload.hex()
    
    print(f"[*] Dispatching pure, unmutated hex:\n{pure_hex}\n")
